...
pub async fn remove_label_from_feedback(
    ...
    // Remove label if no feedback is associated with it
    if sqlx::query!(
        "DELETE FROM labels WHERE id = $1 AND NOT EXISTS (SELECT * FROM feedback_labels WHERE label_id = $2)",
        label_id,
        label_id
    )
    .execute(&pool)
    ...
    .is_err()
    {
        return (StatusCode::INTERNAL_SERVER_ERROR, Json(serde_json::json!({"error": "Failed to remove label"}))).into_response();
    }
